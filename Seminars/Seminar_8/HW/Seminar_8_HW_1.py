...
def transform_data(ti):
    ...
    booking_df['booking_date'] = pd.to_datetime(booking_df['booking_date'], errors='coerce').dt.strftime('%Y-%m-%d')
...
def load_to_mysql(ti):
    ...
    combined_df['booking_date'] = pd.to_datetime(combined_df['booking_date'], errors='coerce')  # Преобразование дат в правильный формат
    ...
    combined_df['booking_date'].fillna(pd.NaT, inplace=True)  # Заполнение пропущенных значений для booking_date
