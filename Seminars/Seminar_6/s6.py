...
t0=time.time()
con=create_engine("mysql://Airflow:Airflow1!@localhost/spark")
...
if 1==11:
    ...
    df.withColumn("id",col("id").cast("int"))\
        ...
        .write.format("jdbc").option("url","jdbc:mysql://localhost:33061/spark?user=Airflow&password=Airflow1!")\
	...
	.option("usePlainNumberFormat","true")\
        ...
        .option("header", "true")\
        .format("excel")\
        .load("/home/sergei/s4.xlsx").where(col("title") == "news")\
        .withColumn("dt", current_timestamp())\
        .write.format("jdbc").option("url","jdbc:mysql://localhost:33061/spark?user=Airflow&password=Airflow1!")\
        .option("driver", "com.mysql.cj.jdbc.Driver").option("dbtable", "tasketl4a")\
        .mode("append").save()
...
df1 = spark.read.format("com.crealytics.spark.excel")\
	...
	.option("usePlainNumberFormat","true")\
        ...
        .option("header", "true")\
        .format("excel")\
        .load("/home/sergei/s4_2.xlsx").limit(1000)\
        .withColumn("проценты", sum1(col("Платеж по процентам")).over(w))\
        .withColumn("долг", sum1(col("Платеж по основному долгу")).over(w))
df1.write.format("jdbc").option("url","jdbc:mysql://localhost:33061/spark?user=Airflow&password=Airflow1!")\
